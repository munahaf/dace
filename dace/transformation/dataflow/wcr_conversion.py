    ...
    def apply(self, state: SDFGState, sdfg: SDFG):
        ...
        if self.expr_index == 0:
            ...
            if state.in_degree(input) > 0:
                ...
                subgraph: StateSubgraphView = StateSubgraphView(state, subgraph_nodes)
                ...
                for edge in subgraph.edges():
                    new_edge = nested_state.add_edge(node_map[edge.src], edge.src_conn, node_map[edge.dst], edge.dst_conn, edge.data)
                    if edge == inedge:
                        new_inedge = new_edge
                ...
                for in_access in in_access_nodes:
                    ...
                    state.add_edge(in_access, None, nested_sdfg_node, in_access.data, Memlet.from_array(in_access.data, sdfg.arrays[in_access.data]))

                for out_access in out_access_nodes:
                    nested_sdfg_node.add_in_connector(out_access.data)
                    state.add_edge(nested_sdfg_node, out_access.data, out_access, None, Memlet.from_array(out_access.data, sdfg.arrays[out_access.data]))
